...
class ProductListAdapter implements ProductListRepository {
  ...
  Future<List<Product>> getProductListByProduct(String product) async{
    ...
    ProductListResponse fastapi = await service.getProductList(Constant.url_back_fast_api, product);

    ProductListResponse springBoot = await service.getProductList(Constant.url_back_spring_boot, product);

    ProductListResponse flask = await service.getProductList(Constant.url_back_flask, product);

    if(fastapi.total != 0){
      fastapi.items.forEach((element) {productList.add(mapperProductResponseToProduct(element, fastapi.seller));});
    }

    if(springBoot.total != 0){
      springBoot.items.forEach((element) {productList.add(mapperProductResponseToProduct(element, springBoot.seller));});
    } 

    if(flask.total != 0){
      flask.items.forEach((element) {productList.add(mapperProductResponseToProduct(element, flask.seller));});
    } 
    
    return productList;
  }

  Product mapperProductResponseToProduct(ProductResponse productResponse, SellerResponse sellerResponse){
    return new Product(productResponse.id, 
